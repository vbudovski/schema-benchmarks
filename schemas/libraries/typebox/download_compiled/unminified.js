...
function FromValue$3(value) {
	return value instanceof RegExp ? FromRegExp$1(value) : IsGuard$1(value) ? FromGuard(value) : IsArray$1(value) ? FromArray$10(value) : IsObject$1(value) ? FromObject$14(value) : FromUnknown(value);
}
/**
...
function FromValue$2(value) {
	return IsTypeArray(value) ? FromTypeArray(value) : IsDate$1(value) ? FromDate(value) : IsRegExp(value) ? FromRegExp(value) : IsBoolean$1(value) ? FromBoolean$5(value.valueOf()) : IsString$1(value) ? FromString$6(value.valueOf()) : IsNumber$1(value) ? FromNumber$4(value.valueOf()) : IsIEEE754(value) ? FromNumber$4(value) : IsArray$1(value) ? FromArray$9(value) : IsBoolean$2(value) ? FromBoolean$5(value) : IsBigInt$1(value) ? FromBigInt$5(value) : IsConstructor$1(value) ? FromConstructor(value) : IsNull$1(value) ? FromNull$1(value) : IsObject$1(value) ? FromObject$13(value) : IsString$2(value) ? FromString$6(value) : IsSymbol$1(value) ? FromSymbol(value) : IsUndefined$1(value) ? FromUndefined$1(value) : IsFunction$1(value) ? FromFunction(value) : Unreachable$1();
}
...
function FromType$21(variants, type) {
	return IsUnion(type) ? FromUnion$11(variants, type.anyOf) : IsLiteral(type) ? FromLiteral$4(variants, type.const) : Unreachable$1();
}
...
function CompositeProperties(left, right) {
	return [.../* @__PURE__ */ new Set([...Keys(right), ...Keys(left)])].reduce((result, key) => {
		return {
			...result,
...
function _Mapped_Mapping(input) {
	return IsArray$1(input[6]) && IsEqual(input[6].length, 1) ? MappedDeferred(Identifier(input[3]), input[5], input[6][0], ApplyReadonly(input[1], ApplyOptional(input[8], input[10]))) : MappedDeferred(Identifier(input[3]), input[5], Ref$1(input[3]), ApplyReadonly(input[1], ApplyOptional(input[8], input[10])));
}
...
function FromObject$12(inferred, properties) {
...
function ExtendsRecord(inferred, leftPattern, leftValue, right) {
	return IsRecord(right) ? FromRecord$5(inferred, RecordPatternToType(leftPattern), leftValue, RecordPatternToType(RecordPattern(right)), RecordValue(right)) : IsObject(right) ? FromObject$12(inferred, right.properties) : IsAny(right) ? ExtendsTrue(inferred) : IsUnknown(right) ? ExtendsTrue(inferred) : ExtendsFalse();
}
//#endregion
...
function FromType$20(stack, context, type) {
	return IsRef$1(type) ? FromRef$7(stack, context, type.$ref) : IsArray(type) ? FromType$20(stack, context, type.items) : IsAsyncIterator(type) ? FromType$20(stack, context, type.iteratorItems) : IsConstructor(type) ? FromTypes$4(stack, context, [...type.parameters, type.instanceType]) : IsFunction(type) ? FromTypes$4(stack, context, [...type.parameters, type.returnType]) : IsInterfaceDeferred(type) ? FromProperties$4(stack, context, type.parameters[1]) : IsIntersect(type) ? FromTypes$4(stack, context, type.allOf) : IsIterator(type) ? FromType$20(stack, context, type.iteratorItems) : IsObject(type) ? FromProperties$4(stack, context, type.properties) : IsPromise(type) ? FromType$20(stack, context, type.item) : IsUnion(type) ? FromTypes$4(stack, context, type.anyOf) : IsTuple(type) ? FromTypes$4(stack, context, type.items) : IsRecord(type) ? FromType$20(stack, context, RecordValue(type)) : false;
}
...
function FromType$19(context, type, result) {
	return IsRef$1(type) ? FromRef$6(context, type.$ref, result) : IsArray(type) ? FromType$19(context, type.items, result) : IsAsyncIterator(type) ? FromType$19(context, type.iteratorItems, result) : IsConstructor(type) ? FromTypes$3(context, [...type.parameters, type.instanceType], result) : IsFunction(type) ? FromTypes$3(context, [...type.parameters, type.returnType], result) : IsInterfaceDeferred(type) ? FromProperties$3(context, type.parameters[1], result) : IsIntersect(type) ? FromTypes$3(context, type.allOf, result) : IsIterator(type) ? FromType$19(context, type.iteratorItems, result) : IsObject(type) ? FromProperties$3(context, type.properties, result) : IsPromise(type) ? FromType$19(context, type.item, result) : IsUnion(type) ? FromTypes$3(context, type.anyOf, result) : IsTuple(type) ? FromTypes$3(context, type.items, result) : IsRecord(type) ? FromType$19(context, RecordValue(type), result) : result;
}
...
function FromType$18(type) {
	return IsRef$1(type) ? FromRef$5(type.$ref) : IsArray(type) ? _Array_(FromType$18(type.items), ArrayOptions(type)) : IsAsyncIterator(type) ? AsyncIterator(FromType$18(type.iteratorItems)) : IsConstructor(type) ? Constructor(FromTypes$2(type.parameters), FromType$18(type.instanceType)) : IsFunction(type) ? _Function_$1(FromTypes$2(type.parameters), FromType$18(type.returnType)) : IsIntersect(type) ? Intersect(FromTypes$2(type.allOf)) : IsIterator(type) ? Iterator(FromType$18(type.iteratorItems)) : IsObject(type) ? _Object_$1(FromProperties$2(type.properties)) : IsPromise(type) ? _Promise_(FromType$18(type.item)) : IsRecord(type) ? Record(RecordKey(type), FromType$18(RecordValue(type))) : IsUnion(type) ? Union(FromTypes$2(type.anyOf)) : IsTuple(type) ? Tuple(FromTypes$2(type.items)) : type;
}
...
function FromUnion$9(types) {
	return ShiftLeft(types, (left, right) => ReduceVariants(right, FromType$15(left)), () => Unreachable$1());
}
//#endregion
...
function FromType$13(properties, type) {
	return IsArray(type) ? _Array_(FromType$13(properties, type.items)) : IsAsyncIterator(type) ? AsyncIterator(FromType$13(properties, type.iteratorItems)) : IsConstructor(type) ? Constructor(FromTypes$1(properties, type.parameters), FromType$13(properties, type.instanceType)) : IsFunction(type) ? _Function_$1(FromTypes$1(properties, type.parameters), FromType$13(properties, type.returnType)) : IsIterator(type) ? Iterator(FromType$13(properties, type.iteratorItems)) : IsPromise(type) ? _Promise_(FromType$13(properties, type.item)) : IsTuple(type) ? Tuple(FromTypes$1(properties, type.items)) : IsUnion(type) ? Union(FromTypes$1(properties, type.anyOf)) : IsIntersect(type) ? Intersect(FromTypes$1(properties, type.allOf)) : IsThis(type) ? _Object_$1(properties) : type;
}
...
function FromType$8(type) {
	return IsCyclic(type) ? FromCyclic$6(type.$defs, type.$ref) : IsDependent(type) ? FromDependent$2(type.if, type.then, type.else) : IsIntersect(type) ? FromIntersect$6(type.allOf) : IsUnion(type) ? FromUnion$6(type.anyOf) : IsObject(type) ? FromObject$8(type.properties) : _Object_$1({});
}
//#endregion
...
function FromType$5(type) {
	return IsCyclic(type) ? FromCyclic$4(type.$defs, type.$ref) : IsDependent(type) ? FromDependent(type.if, type.then, type.else) : IsIntersect(type) ? FromIntersect$4(type.allOf) : IsUnion(type) ? FromUnion$4(type.anyOf) : IsObject(type) ? FromObject$6(type.properties) : _Object_$1({});
}
//#endregion
...
function Comparer(left, right) {
	...
	return IsEqual(compareResult, "right-inside") ? 1 : IsEqual(compareResult, "disjoint") ? 1 : 0;
}
...
function Script(...args) {
	...
	return Update$1(IsArray$1(result) && IsEqual(result.length, 2) ? InstantiateType(context, State([], []), result[0]) : Never(), {}, options);
}
//#endregion
...
function FromType$3(type) {
	return Modifiers(type, IsArray(type) ? _Array_(FromType$3(type.items), ArrayOptions(type)) : IsIntersect(type) ? Intersect(FromTypes(type.allOf)) : IsUnion(type) ? Union(FromPriorityTypes(type.anyOf)) : IsObject(type) ? _Object_$1(FromProperties$1(type.properties)) : IsRecord(type) ? Record(RecordKey(type), FromType$3(RecordValue(type))) : IsTuple(type) ? Tuple(FromTypes(type.items)) : type);
}
/**
