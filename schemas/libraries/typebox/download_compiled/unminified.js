...
function FromRef$6(context, ref, result) {
	return result.includes(ref) ? result : ref in context ? FromType$18(context, context[ref], [...result, ref]) : Unreachable$1();
}
...
function FromType$17(type) {
	return IsRef$1(type) ? FromRef$5(type.$ref) : IsArray(type) ? _Array_(FromType$17(type.items), ArrayOptions(type)) : IsAsyncIterator(type) ? AsyncIterator(FromType$17(type.iteratorItems)) : IsConstructor(type) ? Constructor$1(FromTypes$1(type.parameters), FromType$17(type.instanceType)) : IsFunction(type) ? _Function_$1(FromTypes$1(type.parameters), FromType$17(type.returnType)) : IsIntersect(type) ? Intersect(FromTypes$1(type.allOf)) : IsIterator(type) ? Iterator(FromType$17(type.iteratorItems)) : IsObject(type) ? _Object_$1(FromProperties$1(type.properties)) : IsPromise(type) ? _Promise_(FromType$17(type.item)) : IsRecord(type) ? Record(RecordKey(type), FromType$17(RecordValue(type))) : IsUnion(type) ? Union(FromTypes$1(type.anyOf)) : IsTuple(type) ? Tuple$1(FromTypes$1(type.items)) : type;
}
...
function FromTuple$7(types) {
	return FromType$14(TupleToObject(Tuple$1(types)));
}
//#endregion
...
function FromUnion$9(types) {
	return TakeLeft(types, (left, right) => ReduceVariants(right, FromType$14(left)), () => Unreachable$1());
}
//#endregion
...
function CollapseToObject(type) {
	return _Object_$1(FromType$14(type));
}
//#endregion
